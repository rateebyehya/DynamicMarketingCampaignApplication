...
def getLLMResponse(query, age_option, tasktype_option):
    ...
    from dotenv import load_dotenv #This will load all the key information (OPENAI_API_KEY)
    ...
    prefix="""You are a {template_ageoption}. Please {template_tasktypeoption}
    Here are some examples: 
    """
    ...
    #use example selector for maximum length to avoid high credit card bills 
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["template_userInput", "template_ageoption", "template_tasktypeoption"],
        example_separator="\n"
    )

    #query = "What is a house?" 
    query = for_input #from streamlit 
    print(new_prompt_template.format(
        template_userInput=query, 
        template_ageoption=age_option,
        template_tasktypeoption=tasktype_option ))
...
for_input = st.text_area("Enter text", height = 50) #helps take input from the user 

#The following allows you to create selectbox in streamlit!! 
#there are a lot of controls in streamlit which you can use to make the UI interphase look better!!
...
#The reason why we have different keys for different selectboxes is to differentiate them from each other

numberOfWords= st.slider('Words limit', 1, 200, 25)
#Word limit is for credit card billing problem that we talked about earlier

submit = st.button("Generate")
#NOW WE HAVE THE UI IMPLEMENTATION READY FOR USE. IT IS TIME TO IMPLEMENT THE ACTUAL CODE. 
if submit: 
    st.write(getLLMResponse(for_input, age_option, tasktype_option)) #should define the function before 
